module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		concat: {
			...
			jquery: {
				src: [
					'<%= dirs.src %>/renderer/canvas.js',
					'<%= dirs.src %>/<%= cfg.filename %>.js',
					'<%= dirs.src %>/jquery-2.2.3.plugin.js'
				],
				dest: '<%= dirs.tmp %>/jquery-2.2.3.<%= cfg.filename %>.js'
			},
			angular: {
				src: [
		...
		usebanner: {
			...
			files: {
				src: [
					...
					'<%= dirs.dest %>/jquery-2.2.3.<%= cfg.filename %>.js',
		...
		uglify: {
			dist: {
				...
				files: {
					...
					'dist/jquery.<%= cfg.filename %>.min.js': ['dist/jquery-2.2.3.<%= cfg.filename %>.js'],
		...
		umd: {
			...
			jquery: {
				src: '<%= dirs.tmp %>/jquery-2.2.3.<%= cfg.filename %>.js',
				dest: '<%= dirs.dest %>/jquery-2.2.3.<%= cfg.filename %>.js',
				deps: {
					'default': ['$'],
					amd: ['jquery'],
					cjs: ['jquery'],
